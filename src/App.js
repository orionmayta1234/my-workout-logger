...
import { 
    ...
    Home, 
    Calendar, Edit, 
    LogOut, SkipForward, RotateCcw, Replace, MessageSquareWarning, AlertTriangle, History, GripVertical 
    // Bell, // Removed as it was unused
} from 'lucide-react';
...
function App() {
                ...
                if (pageHiddenTimestampRef.current) {
                    ...
                    setTimerSeconds(prevSeconds => {
                        ...
                        if (newSeconds <= 0) {
                            ...
                            if (notificationPermission === "granted") {
                                new Notification("FitTrack Timer", {
                                    body: "Your rest timer is up! (Corrected after tab focus)",
                                    icon: "/dumbbell.svg"
                                });
                            }
                            return 0;
    ...
    }, [db, userId, showInfoModal]); // Removed appId from deps
    ...
    const handleExerciseTouchEndCallback = useCallback(() => { document.removeEventListener('touchmove', handleExerciseTouchMoveCallback); document.removeEventListener('touchend', handleExerciseTouchEndCallback); if (draggedElementRef.current) { draggedElementRef.current.classList.remove('opacity-60', 'ring-2', 'ring-pink-500', 'z-50', 'dragging-touch'); } document.querySelectorAll('.exercise-item.drop-target-touch').forEach(el => el.classList.remove('drop-target-touch')); const sourceIndex = touchDragItemIndexRef.current; const targetIndex = touchDragOverItemIndexRef.current; if (editingWorkout && sourceIndex !== null && targetIndex !== null && sourceIndex !== targetIndex) { performReorderAndUpdateFirestore(sourceIndex, targetIndex, 'exercise', editingWorkout.exercises, (newExercises) => setEditingWorkout(prev => ({...prev, exercises: newExercises})), null); } touchDragItemIndexRef.current = null; touchDragOverItemIndexRef.current = null; draggedElementRef.current = null; }, [editingWorkout, setEditingWorkout, performReorderAndUpdateFirestore, handleExerciseTouchMoveCallback]); // Added handleExerciseTouchMoveCallback
