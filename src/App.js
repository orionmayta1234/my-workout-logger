...
import { 
    ...
    Home, 
    Calendar, Edit, 
    LogOut, SkipForward, RotateCcw, Replace, MessageSquareWarning, AlertTriangle, History, GripVertical, Bell
} from 'lucide-react';
...
function App() {
    ...
    const pageHiddenTimestampRef = useRef(null); // For background timer correction
    ...
    useEffect(() => {
        ...
        } else if (Notification.permission !== "denied") { // Only request if not denied
    ...
    // --- Timer Logic with Background Correction ---
    useEffect(() => {
                ...
                setTimerSeconds(prev => {
                    if (prev <= 1) {
                        ...
                        if (notificationPermission === "granted") {
                            new Notification("FitTrack Timer", {
                                body: "Your rest timer is up! Time for the next set.",
                                icon: "/dumbbell.svg" 
                            });
                        }
                        return 0;
    ...
    }, [timerActive, timerPaused, notificationPermission]);

    // --- Page Visibility for Timer Correction ---
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (!timerActive || timerPaused) return; // Only adjust if timer is active and not paused

            if (document.hidden) {
                pageHiddenTimestampRef.current = Date.now();
            } else {
                if (pageHiddenTimestampRef.current) {
                    const hiddenDurationMs = Date.now() - pageHiddenTimestampRef.current;
                    const hiddenDurationSeconds = Math.round(hiddenDurationMs / 1000);
                    
                    setTimerSeconds(prevSeconds => {
                        const newSeconds = prevSeconds - hiddenDurationSeconds;
                        if (newSeconds <= 0) {
                            clearInterval(timerIntervalRef.current);
                            setTimerActive(false);
                            if (notificationPermission === "granted") {
                                new Notification("FitTrack Timer", {
                                    body: "Your rest timer is up!",
                                    icon: "/dumbbell.svg"
                                });
                            }
                            return 0;
                    ...
                    pageHiddenTimestampRef.current = null; 
                }
            }
        };
    ...
    }, [timerActive, timerPaused, notificationPermission]); // Dependencies for visibility handler


    const startTimer = useCallback((duration = 180) => { setTimerSeconds(duration); setTimerActive(true); setTimerPaused(false); pageHiddenTimestampRef.current = null; }, []); // Reset hidden timestamp on new timer start
    const pauseTimer = useCallback(() => { setTimerPaused(true); clearInterval(timerIntervalRef.current); pageHiddenTimestampRef.current = null; }, []); // Clear hidden timestamp on pause
    const resumeTimer = useCallback(() => { setTimerPaused(false); pageHiddenTimestampRef.current = null; }, []); // Reset hidden timestamp on resume
    ...
    const performReorderAndUpdateFirestore = useCallback(async (sourceIndex, targetIndex, itemType, currentItems, setItemsCallback, collectionName) => {
        if (sourceIndex === null || targetIndex === null || sourceIndex === targetIndex) {
            return; 
        }
        const reorderedItems = [...currentItems];
        const [draggedActualItem] = reorderedItems.splice(sourceIndex, 1);
        reorderedItems.splice(targetIndex, 0, draggedActualItem);

        setItemsCallback(reorderedItems); 

        if (itemType === 'workout plan' && db && userId && collectionName) {
            const batch = writeBatch(db);
            const collRef = collection(db, `artifacts/${appId}/users/${userId}/${collectionName}`);
            reorderedItems.forEach((item, newOrderIndex) => {
                if (item.id) {
            ...
            try {
                await batch.commit();
                console.log(`${itemType} order updated in Firestore.`);
            } catch (error) {
                console.error(`Error updating ${itemType} order:`, error);
                showInfoModal(`Failed to save new order for ${itemType}s. Please try again.`);
            }
        } else if (itemType === 'exercise') {
            console.log("Exercise order updated in local state.");
        }
    ...
    const handleExerciseDragEnd = useCallback((e) => { /* ... as before ... */ if(e.currentTarget) e.currentTarget.classList.remove('opacity-50', 'ring-2', 'ring-pink-500'); draggedItemIndex.current = null; }, []);

    const handleExerciseTouchStart = useCallback((e, index) => { touchDragItemIndexRef.current = index; draggedElementRef.current = e.currentTarget; if (e.currentTarget) e.currentTarget.classList.add('opacity-60', 'ring-2', 'ring-pink-500', 'z-50', 'dragging-touch'); document.addEventListener('touchmove', handleExerciseTouchMove, { passive: false }); document.addEventListener('touchend', handleExerciseTouchEnd); }, []);
    const handleExerciseTouchMove = useCallback((e) => { if (touchDragItemIndexRef.current === null || !draggedElementRef.current || !editingWorkout) return; e.preventDefault(); const touch = e.touches[0]; const elementUnderTouch = document.elementFromPoint(touch.clientX, touch.clientY); document.querySelectorAll('.exercise-item.drop-target-touch').forEach(el => el.classList.remove('drop-target-touch')); touchDragOverItemIndexRef.current = null; if (elementUnderTouch) { const dropTarget = elementUnderTouch.closest('.exercise-item'); if (dropTarget && dropTarget !== draggedElementRef.current) { dropTarget.classList.add('drop-target-touch'); const targetId = dropTarget.id.replace('exercise-item-', ''); const targetIndex = editingWorkout.exercises.findIndex(ex => ex.id === targetId); if (targetIndex !== -1) { touchDragOverItemIndexRef.current = targetIndex; } } } }, [editingWorkout]);
    const handleExerciseTouchEnd = useCallback(() => { document.removeEventListener('touchmove', handleExerciseTouchMove); document.removeEventListener('touchend', handleExerciseTouchEnd); if (draggedElementRef.current) { draggedElementRef.current.classList.remove('opacity-60', 'ring-2', 'ring-pink-500', 'z-50', 'dragging-touch'); } document.querySelectorAll('.exercise-item.drop-target-touch').forEach(el => el.classList.remove('drop-target-touch')); const sourceIndex = touchDragItemIndexRef.current; const targetIndex = touchDragOverItemIndexRef.current; if (editingWorkout && sourceIndex !== null && targetIndex !== null && sourceIndex !== targetIndex) { performReorderAndUpdateFirestore(sourceIndex, targetIndex, 'exercise', editingWorkout.exercises, (newExercises) => setEditingWorkout(prev => ({...prev, exercises: newExercises})), null); } touchDragItemIndexRef.current = null; touchDragOverItemIndexRef.current = null; draggedElementRef.current = null; }, [editingWorkout, setEditingWorkout, performReorderAndUpdateFirestore]);
